...
class TestClientSummaryIntegration:
    ...
    def test_summary_streaming_workflow(self, api_key, base_url):
        ...
        with patch.object(client.session, "post") as mock_post:
            ...
            summary = client.Summary(url="https://example.com/article", stream_callback=callback)
...
class TestOutputFormats:
    ...
    def test_article_html_vs_text_output(self, api_key, base_url, mock_response):
        ...
        with patch.object(client.session, "post") as mock_post:
            ...
            article_html = client.Article(url="https://example.com/article", output="html")
            assert article_html.body == "<p>HTML content</p>"

            # Test text output
            response = mock_response(status_code=200, json_data=text_data)
            mock_post.return_value = response
            article_text = client.Article(url="https://example.com/article", output="text")
    ...
    def test_pdf_html_vs_text_output(self, api_key, base_url, mock_response):
        ...
        with patch.object(client.session, "get") as mock_get:
            ...
            pdf_html = client.PDF(url="https://example.com/document.pdf", output="html")
            assert pdf_html.body == "<p>PDF HTML</p>"

            # Test text output
            response = mock_response(status_code=200, json_data=text_data)
            mock_get.return_value = response
            pdf_text = client.PDF(url="https://example.com/document.pdf", output="text")
